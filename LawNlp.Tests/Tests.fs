...
[<Fact>]
let ``Разделение переходов когда используется точка как разделитель даты и номера`` () =
    let узел = HtmlNode.NewText("Постановление Правления Национального Банка Республики Казахстан от 2001.04.09. N 89. Зарегистрировано в Министерстве юстиции Республики Казахстан 17.04.2001 г. за N 1466. Утратило силу - постановлением Правления Национального Банка РК от 13 сентября 2004 года N 119.")
    let parsedData = разобратьПереходы узел
    verify parsedData